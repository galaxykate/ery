...
function toOrbitalCamera(camera) {

	// ORBITAL CAMERA
	camera.theta = Math.PI / 2;
	camera.center = new Vector();
	camera.focus = new Vector();
	camera.focus3 = new THREE.Vector3();
	camera.phi = .4;
	camera.radius = 400;
	...
	camera.update = function() {
		...
		var p = new Vector(camera.center);
		p.addSpherical(camera.radius, camera.theta, camera.phi);

		camera.focus.cloneInto(camera.focus3)
		camera.position.set(p.x, p.y, p.z);
		camera.lookAt(camera.focus3);
	}

	camera.update();
	camera.updateScreenPos = function(worldPos, screenPos) {

		var widthHalf = camera.renderer.context.canvas.width / 2;
		var heightHalf = camera.renderer.context.canvas.height / 2;

		let scale = 1 * heightHalf / widthHalf;

		// Clone the worldPos and set this to it
		let pos = new THREE.Vector3()
		pos.set(worldPos.x, worldPos.y, worldPos.z)

		pos.project(camera);
		screenPos.x = scale * (pos.x * widthHalf);
		screenPos.y = -scale * (pos.y * heightHalf);
	};
	return camera;
}

var utilities = {
	createThree: function(holder, onInit, onUpdate) {
		...
		let h = holder.height();
		console.log(w, h)
		var time = new AnimTime();


		camera = new THREE.PerspectiveCamera(25, window.innerWidth / window.innerHeight, 100, 1000);
		...
		onInit(scene, camera, time)
		animate();



		function bookmark(x, y) {
			camera.dragStart = new Vector(x, y);
			camera.bookmarkPhi = camera.phi;
			camera.bookmarkTheta = camera.theta;
		}

		function bookmarkOffset(x, y) {
			camera.phi = 1.2 + y * -.004;
			camera.theta = -x * .008;
		}
...
KColor.prototype.toCSS = function() {
	...
	let val = (this.h * 360).toFixed(0) + ", " + (this.s * 100).toFixed(0) + "%, " + (this.l * 100).toFixed() + "%";
	if (this.a < 1)
		return "hsla(" + val + ", " + this.a + ")";
	return "hsl(" + val + ")";
}
...
function mapObj(obj, fxn) {
	let keys = Object.keys(obj);
	let obj2 = {};

	keys.forEach((key) => {
		obj2[key] = fxn(obj[key], key, obj);
	})
	return obj2;
}

function objToArray(obj, fxn) {
	let keys = Object.keys(obj);
	return keys.map((key) => {
		return fxn(obj[key], key)
	})
}
